...
GH_BASE="https://github.com/Crystal-Kitsune-Studios/MintKit/releases/download/v$VERSION"
# <title> tag
...
# Download button href — point to GitHub releases (files are generated by the workflow)
sed -i "s|href=\"https://github.com/Crystal-Kitsune-Studios/MintKit/releases/download/v[^/]*/[^\"]*\"|href=\"$GH_BASE/$IMG_XZ\"|g" "$DOWNLOAD_HTML"
sed -i "s|href=\"/dist/[^\"]*\"|href=\"$GH_BASE/$IMG_XZ\"|g" "$DOWNLOAD_HTML"
# Torrent link
sed -i "s|href=\"https://github.com/Crystal-Kitsune-Studios/MintKit/releases/download/v[^/]*/mintkit-v[^\"]*\.torrent\"|href=\"$GH_BASE/mintkit-v$VERSION.torrent\"|g" "$DOWNLOAD_HTML"
...
# Build release title — include codename if provided
if [[ -n "$VERSION_NAME" ]]; then
  RELEASE_TITLE="MintKit v$VERSION \"$VERSION_NAME\""
else
  RELEASE_TITLE="MintKit v$VERSION"
fi
